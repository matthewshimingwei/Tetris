...
SHAPE_TYPE testShape[] = { S_Z, S_S, S_O };
SHAPE_TYPE testShape2[] = { S_J, S_Z, S_J, S_Z, S_T, S_I, S_J, S_O, S_I, S_Z };

int eliminate_score[] = { 0, 100, 200, 400, 800, 0, 0, 0, 0 };

void GenerateTestShapeList(SHAPE_TYPE *shapes, int count, std::vector<SHAPE_TYPE>& sl)
{
    sl.clear();
...
void GenerateShapeList(int count, std::vector<SHAPE_TYPE>& sl)
{
    SHAPE_TYPE last = S_NULL;
    ...
    while(i < count)
    {
        SHAPE_TYPE t = static_cast<SHAPE_TYPE>(rand() % SHAPE_COUNT + 1);
        if(t != last)
        {
            sl.push_back(t);
            last = t;
...
int EvaluateShape(RUSSIA_GAME *game, B_SHAPE *bs, EVA_RESULT *result)
{
    ...
    for(int col = 0; col < (GAME_COLUMN - bs->width + 1); col++)
    {
        int row = start_row;
...
bool ComputerAIPlayer(RUSSIA_GAME *game, SHAPE_TYPE s)
{
    bool res_find = false;
...
{
    memset(game->board, S_NULL, BOARD_ROW * BOARD_COLUMN * sizeof(int));
    ...
    {
        game->board[i][0] = S_B;
        game->board[i][BOARD_COLUMN - 1] = S_B;
    }
...
void PrintGame(RUSSIA_GAME *game, SHAPE_TYPE next)
{
    for(int i = 0; i < BOARD_ROW; i++)
    {
...
{
    std::vector<SHAPE_TYPE> shape_list;
    RUSSIA_GAME game;
